...
BofIoConnectionManager::BofIoConnectionManager(const BOF_IO_CONNECTION_MANAGER_PARAM &_rBofIoConnectionManagerParam_X)
	: BofThread(_rBofIoConnectionManagerParam_X.Name_S), mpBofIoAsyncCmdCollection(nullptr), mBofIoConnectionManagerParam_X(_rBofIoConnectionManagerParam_X), mUvWalkResult_S(""),
	  mpuLoop_X(std::make_unique<uv_loop_t>()), mpuAsync_X(std::make_unique<uv_async_t>()), mpuTimer_X(std::make_unique<uv_timer_t>()), mpIoAsyncRequestCollection(nullptr),
	  mpUvConnectionCollection(nullptr), mLastLockerName_S("")
{
	...
	mErrorCode_E = Bof_CreateMutex(_rBofIoConnectionManagerParam_X.Name_S + "_mtx", true, mConnectionMtx);
	...
	if (mErrorCode_E == BOFERR_NO_ERROR)
	{
		...
		if (mErrorCode_E == BOFERR_NO_ERROR)
		{
			mErrorCode_E = LaunchThread(false, 0, 0x40000, 0, mBofIoConnectionManagerParam_X.Policy_S32, mBofIoConnectionManagerParam_X.Priority_S32,
			                            mBofIoConnectionManagerParam_X.StartStopTimeoutInMs_U32);
		}
	}
}
...
BOFERR BofIoConnectionManager::PushDisconnect(const std::string &_rDbgName_S, BOF_UV_CONNECTION *_pUvConnection_X, bool _CancelPendingOp_B)
{
	...
	//DBG_OUT("[DISC] Disconnect pUv %p pCon %p %s of %s IoState %s Closing %s NbPend %d CancelPendingOp %d InUse %X\n", _pUvConnection_X, _pUvConnection_X ? _pUvConnection_X->pIBofIoConnection : nullptr, _rDbgName_S.c_str(), _pUvConnection_X ? _pUvConnection_X->Name_S.c_str() : "???????",	_pUvConnection_X ? (_pUvConnection_X->pIBofIoConnection ? _pUvConnection_X->pIBofIoConnection->IoStateString().c_str() : "???????") : "???????", _pUvConnection_X ? _pUvConnection_X->Closing_B ? "True" : "False" : "???", mUvDisconnectingConnectionCollection.size(), _CancelPendingOp_B, Rts_E ? 0 : 1);
...
std::string BofIoConnectionManager::BuildConnectionInfo()
{
	...
	while (pUvConnection_X)
	{
		...
		Rts_S += Bof_Sprintf("%03d %-32s %08p %08p %08p %-12s %08X %06d %06d %s %08X %08X %-42s ", Index_U32++, pUvConnection_X->Name_S.c_str(), pUvConnection_X->pIBofIoConnection,
		                     pUvConnection_X->pUvIoConnect, pUvConnection_X->pUvIoChannel,
		                     pUvConnection_X->pIBofIoConnection ? pUvConnection_X->pIBofIoConnection->IoStateString().c_str() : "Nullptr", pUvConnection_X->StartConnectTime_U32, Delta_U32,
		                     pUvConnection_X->IoChannelParam_X.ConnectTimeoutInMs_U32,
		                     IsUdp_B ? "true " : "false", pUvConnection_X->IoChannelParam_X.RcvBufferSize_U32, pUvConnection_X->IoChannelParam_X.SndBufferSize_U32,
		                     pUvConnection_X->IoChannelParam_X.Address_S.c_str());
		if (IsUdp_B)
		{
			Rts_S += Bof_Sprintf("%s %s %s %d\n", pUvConnection_X->IoChannelParam_X.IO_CHANNEL.UDP.Broadcast_B ? "true " : "false",
			                     pUvConnection_X->IoChannelParam_X.IO_CHANNEL.UDP.MulticastLoop_B ? "true " : "false",
	...
	while (pUvConnection_X)
	{
		if (pUvConnection_X->pIBofIoConnection)
		{
			...
			Rts_S += Bof_Sprintf("%03d %s %s %s %-30s %-30s %-30s %08p %08p %s\n", Index_U32++, pUvConnection_X->pIBofIoConnection->IsConnected() ? "true " : "false",
			                     pUvConnection_X->pIBofIoConnection->ServerSession() ? "true " : "false",
			                     Bof_ProtocolTypeToString(pUvConnection_X->pIBofIoConnection->ProtocolType()).c_str(),
			                     Bof_IpAddressToString(pUvConnection_X->pIBofIoConnection->LocalAddress(), true, true).c_str(),
			                     Bof_IpAddressToString(pUvConnection_X->pIBofIoConnection->BindAddress(), true, true).c_str(),
			                     Bof_IpAddressToString(pUvConnection_X->pIBofIoConnection->DestinationAddress(), true, true).c_str(),
			                     pUvConnection_X->pIBofIoConnection->IConnectionFactory(), pUvConnection_X->pIBofIoConnection->IoConnectionManager(),
			                     Bof_ErrorCode(pUvConnection_X->pIBofIoConnection->LastErrorCode()));
		}
		else
		{
	...
	if (mUvDisconnectingConnectionCollection.size())
	{
		...
		{
			Rts_S += Bof_Sprintf("%03d %08p %s %08p %s %s\n", Index_U32++, It, It->Closing_B ? "True " : "False", It->pIBofIoConnection,
			                     It->pIBofIoConnection ? It->pIBofIoConnection->IoStateString().c_str() : "???????", It->Name_S.c_str());
		}
	}
...
BOFERR BofIoConnectionManager::Listen(IBofIoConnectionFactory *_pIBofIoConnectionFactory, const BOF_NAMESPACE::IBOF_IO_CONNECTION_PARAM &_rIBofIoConnectionParam_X,
                                      const BOF_IO_CHANNEL_PARAM &_rBofIoChannelParam_X, uint32_t _NbMaxConnection_U32, IBofIoConnection **_ppIBofIoConnection)
{
	BOFERR Rts_E = BOFERR_INVALID_PARAM, Sts_E;
	BOF_UV_CONNECTION *pUvConnection_X;
...
BOFERR BofIoConnectionManager::SendAsyncCmd(bool _Emergency_B, BOF_IO_ASYNC_CMD &_rBofIoAsyncCmd_X)
{
	...
	DBG_OUT("[DISC1] SendAsyncCmd emer %d cmd %d caller %s pUv %p pCon %p %s IoState %s Closing %s NbPend %d Rts %X\n", _Emergency_B, _rBofIoAsyncCmd_X.Cmd_E,
	        _rBofIoAsyncCmd_X.Caller_S.c_str(), _rBofIoAsyncCmd_X.pUvConnection_X, _rBofIoAsyncCmd_X.pUvConnection_X ? _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection : nullptr,
	        ...
	        _rBofIoAsyncCmd_X.pUvConnection_X ? (_rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection ? _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection->IoStateString().c_str() : "???????")
	                                          : "???????", _rBofIoAsyncCmd_X.pUvConnection_X ? _rBofIoAsyncCmd_X.pUvConnection_X->Closing_B ? "True" : "False" : "???",
	        mUvDisconnectingConnectionCollection.size(), Rts_E);

	return Rts_E;
}

// Passive mode if _NbMaxConnection_U32 is different from 0
BOFERR BofIoConnectionManager::OpenUvConnection(IBofIoConnection *_pIBofIoConnection, const std::string &_rName_S, uint32_t _NbMaxConnection_U32, const BOF_IO_CHANNEL_PARAM &_rBofIoChannelParam_X,
                                                BOF_UV_CONNECTION *_pUvConnection_X)
{
	...
	if (_pUvConnection_X)
	{
		...
		if (Rts_E == BOFERR_NO_ERROR)
		{
			...
			if ((BindProtocolType_E != BOF_PROTOCOL_TYPE::BOF_PROTOCOL_SHARED_RAM)
			    && (BindProtocolType_E != BOF_PROTOCOL_TYPE::BOF_PROTOCOL_UNDEF)
			    && (BindProtocolType_E != BOF_PROTOCOL_TYPE::BOF_PROTOCOL_UNKNOWN)
			    && (BindProtocolType_E != BOF_PROTOCOL_TYPE::BOF_PROTOCOL_ZMQ_PGM)
			    && (BindProtocolType_E != BOF_PROTOCOL_TYPE::BOF_PROTOCOL_MAX))
			{
				...
				if (Sts_i == 0)
				{
					...
					SocketAddress_X.Set(false, IsUdp_B ? BOF_SOCK_TYPE::BOF_SOCK_UDP : BOF_SOCK_TYPE::BOF_SOCK_TCP, IsUdp_B ? BOF_PROTOCOL_TYPE::BOF_PROTOCOL_UDP : BOF_PROTOCOL_TYPE::BOF_PROTOCOL_TCP, Ip1_U32,
					                    Ip2_U32, Ip3_U32, Ip4_U32, ntohs(IpAddress_X.sin_port));
...
BOFERR BofIoConnectionManager::OnUvAsyncWrite(const BOF_IO_ASYNC_CMD &_rBofIoAsyncCmd_X)
{
	...
	DBG_OUT("[WRT1] pUv %p pCon %p %s IoState %s Closing %s NbPend %d Data %d:%s Nbbufval %d\n", _rBofIoAsyncCmd_X.pUvConnection_X,
	        _rBofIoAsyncCmd_X.pUvConnection_X ? _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection : nullptr,
	        _rBofIoAsyncCmd_X.pUvConnection_X ? _rBofIoAsyncCmd_X.pUvConnection_X->Name_S.c_str() : "???????",
	        _rBofIoAsyncCmd_X.pUvConnection_X ? (_rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection ? _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection->IoStateString().c_str() : "???????")
	                                          : "???????",
	        ...
	        _rBofIoAsyncCmd_X.pBuffer_X[0].SizeInByte_U32, p, _rBofIoAsyncCmd_X.NbBufferValid_U32);
	...
	if (_rBofIoAsyncCmd_X.NbBufferValid_U32 <= NB_MAX_WRITE_BUFFER_PER_OP)
	{
		...
		DBG_OUT("[WRT2] pUv %p pBofIoAsyncRequest_X %p capa %d/%d\n", _rBofIoAsyncCmd_X.pUvConnection_X, pBofIoAsyncRequest_X, mpIoAsyncRequestCollection->GetNbElementOutOfThePot(),
		        mpIoAsyncRequestCollection->GetCapacity());
		...
		if (pBofIoAsyncRequest_X)
		{
			...
			if (IsUdp_B)
			{
				...
				ALLOC_UV_CALLBACK_DATA(&pBofIoAsyncRequest_X->UdpWrtReq_X, pBofUvCallback_X, (std::string("UdpWrtReq_") + pUvCallback_X->Name_S), false, _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection,
				                       pBofIoAsyncRequest_X, _rBofIoAsyncCmd_X.pUvConnection_X);
			...
			else
			{
				...
				BOF_ASSERT(pUvCallback_X != nullptr);
				ALLOC_UV_CALLBACK_DATA(&pBofIoAsyncRequest_X->TcpWrtReq_X, pBofUvCallback_X, (std::string("TcpWrtReq_") + pUvCallback_X->Name_S), false, _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection,
				                       pBofIoAsyncRequest_X, _rBofIoAsyncCmd_X.pUvConnection_X);
...
BOFERR BofIoConnectionManager::OnUvAsyncDisconnect(const BOF_IO_ASYNC_CMD &_rBofIoAsyncCmd_X)
{
	...
	DBG_OUT("[DISC4] OnUvAsyncDisconnect strm %p pconnect %p puvtcp %p cancel %d iswrt %d qs %d\n", pUvStream_X, pUvConnect_X, pUvTcp_X,
	        _rBofIoAsyncCmd_X.pUvConnection_X->CancelPendingOp_B, uv_is_writable(pUvStream_X), pUvStream_X->write_queue_size);
	...
	if (pUvTcp_X)
	{
		...
		if ((!_rBofIoAsyncCmd_X.pUvConnection_X->CancelPendingOp_B) && (uv_is_writable(pUvStream_X)) && (pUvStream_X->write_queue_size > 0))
		{
			...
			if (pBofIoAsyncRequest_X)
			{
				...
				ALLOC_UV_CALLBACK_DATA(&pBofIoAsyncRequest_X->ShutdownReq_X, pBofUvCallback_X, (std::string("DisReq_") + pUvCallback_X->Name_S), false, _rBofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection,
				                       pBofIoAsyncRequest_X, _rBofIoAsyncCmd_X.pUvConnection_X);
				// DBG_OUT("### shutdown\n");
		...
		else
		{
			...
			DBG_OUT("[DISC8] OnUvAsyncDisconnect strm %p pconnect %p puvtcp %p cancel %d iswrt %d qs %d->Is closing %X\n", pUvStream_X, pUvConnect_X, pUvTcp_X,
			        _rBofIoAsyncCmd_X.pUvConnection_X->CancelPendingOp_B, uv_is_writable(pUvStream_X), pUvStream_X->write_queue_size, uv_is_closing(pHandle));
...
BOFERR BofIoConnectionManager::OnUvWrite(BOF_UV_CALLBACK *_pBofUvCallback_X, int _Status_i)
{
	...
	/*
	   ...
	   DBG_OUT("[%08X] OnUvWrite Rts %x %d buf [0]=%d:%p MustbeFreed %d con %s->%s\n", BOF_NAMESPACE::Bof_GetMsTickCount(), Rts_E, pBofIoAsyncRequest_X->NbUvBufferValid_U32, pBofIoAsyncRequest_X->pUvBuffer_X[0].len, pBofIoAsyncRequest_X->pUvBuffer_X[0].base, pBofIoAsyncRequest_X->pUvBufferMustbeFreed_B[0], pBofIoAsyncRequest_X->pIBofIoConnection->Name().c_str(),p);
	   }
	 */
	for (i_U32 = 0; i_U32 < pBofIoAsyncRequest_X->NbUvBufferValid_U32; i_U32++)
	{
			...
			if (Rts_E == BOFERR_NO_ERROR)
			{
				...
				if (pUvConnection_X->pIBofIoConnection)
				{
					...
					else
					{
						...
						pIpAddress_X = reinterpret_cast< struct sockaddr_in * > (&SockName_X);
						BOF_U32IPADDR_TO_U8IPADDR(pIpAddress_X->sin_addr.s_addr, Ip1_U32, Ip2_U32, Ip3_U32, Ip4_U32);
						SocketAddress_X.Set(false, IsUdp_B ? BOF_SOCK_TYPE::BOF_SOCK_UDP : BOF_SOCK_TYPE::BOF_SOCK_TCP, IsUdp_B ? BOF_PROTOCOL_TYPE::BOF_PROTOCOL_UDP : BOF_PROTOCOL_TYPE::BOF_PROTOCOL_TCP,
						                    Ip1_U32, Ip2_U32, Ip3_U32, Ip4_U32, ntohs(pIpAddress_X->sin_port));
					...
					BOF_U32IPADDR_TO_U8IPADDR(pIpAddress_X->sin_addr.s_addr, Ip1_U32, Ip2_U32, Ip3_U32, Ip4_U32);
					SocketAddress_X.Set(false, IsUdp_B ? BOF_SOCK_TYPE::BOF_SOCK_UDP : BOF_SOCK_TYPE::BOF_SOCK_TCP, IsUdp_B ? BOF_PROTOCOL_TYPE::BOF_PROTOCOL_UDP : BOF_PROTOCOL_TYPE::BOF_PROTOCOL_TCP, Ip1_U32,
					                    Ip2_U32, Ip3_U32, Ip4_U32, ntohs(pIpAddress_X->sin_port));
					pUvConnection_X->pIBofIoConnection->LocalAddress(SocketAddress_X);
					if (IsUdp_B)
					{}
					else
					{
						...
						pUvConnection_X->pIBofIoConnection->BindAddress(SocketAddress_X);
						pUvConnection_X->IoChannelParam_X.Address_S =
							Bof_IpAddressToString(pUvConnection_X->pIBofIoConnection->BindAddress(), true, true) + ";" + Bof_IpAddressToString(pUvConnection_X->pIBofIoConnection->DestinationAddress(), false, true);
					}
					pUvHandle_X = reinterpret_cast< uv_handle_t * > (pUvConnection_X->pUvIoChannel);
					Rts_E = BofIoConnectionManager::S_SetIoChannelOption(IsUdp_B, pUvHandle_X, pUvConnection_X->IoChannelParam_X);
					if (Rts_E == BOFERR_NO_ERROR)
...
// std::string Ip_S = (_pAddress_X != nullptr) ? Bof_SockAddrInToString(*_pAddress_X, true) : "NULL";
	...
	if (Rts_E != BOFERR_NO_ERROR)
	{
		DBG_OUT("### ReadErr %s IoState %s->Post disconnect\n", _pBofUvCallback_X->pUvConnection_X->pIBofIoConnection->Name().c_str(),
		        _pBofUvCallback_X->pUvConnection_X->pIBofIoConnection->IoStateString().c_str());
...
BOFERR BofIoConnectionManager::V_OnUvAsync(BOF_UV_CALLBACK *_pBofUvCallback_X, uv_async_t *_pHandle)
{
	...
	while (!Finish_B)
	{
		...
		if (Rts_E == BOFERR_NO_ERROR)
		{
			...
			DBG_OUT("[DISC2] V_OnUvAsync cmd %d caller %s pUv %p pCon %p %s IoState %s Closing %s NbPend %d Data %d:%s\n", BofIoAsyncCmd_X.Cmd_E,
			        BofIoAsyncCmd_X.Caller_S.c_str(), BofIoAsyncCmd_X.pUvConnection_X, BofIoAsyncCmd_X.pUvConnection_X ? BofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection : nullptr,
			        BofIoAsyncCmd_X.pUvConnection_X ? BofIoAsyncCmd_X.pUvConnection_X->Name_S.c_str() : "???????",
			        BofIoAsyncCmd_X.pUvConnection_X ? (BofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection ? BofIoAsyncCmd_X.pUvConnection_X->pIBofIoConnection->IoStateString().c_str() : "???????")
			                                        : "???????",
			        BofIoAsyncCmd_X.pUvConnection_X ? BofIoAsyncCmd_X.pUvConnection_X->Closing_B ? "True" : "False" : "???", mUvDisconnectingConnectionCollection.size(),
			        BofIoAsyncCmd_X.pBuffer_X[0].SizeInByte_U32, p);
...
BOFERR BofIoConnectionManager::EndOfConnection(BOF_UV_CONNECTION *_pUvConnection_X)
{
	...
	{
		DBG_OUT("[EOC] pUv %p pCon %p Connected %d IoState %s\n", _pUvConnection_X, _pUvConnection_X->pIBofIoConnection, _pUvConnection_X->pIBofIoConnection->IsConnected(),
		        _pUvConnection_X->pIBofIoConnection->IoStateString().c_str());
		if (_pUvConnection_X->pIBofIoConnection->IsConnected())
		{
			...
			DBG_OUT("[EOC] virtual call for pUv %p pCon %p Object %s IoState %s ServerSession %s pIBofIoConnectionFactory %p->Virtual call\n", _pUvConnection_X, _pUvConnection_X->pIBofIoConnection,
			        _pUvConnection_X->pIBofIoConnection->Name().c_str(), _pUvConnection_X->pIBofIoConnection->IoStateString().c_str(), ServerSession_B ? "True" : "False", pIBofIoConnectionFactory);
...
BOFERR BofIoConnectionManager::V_OnUvClose(BOF_UV_CALLBACK *_pBofUvCallback_X, uv_handle_t *_pHandle)
{
	...
	{
		DBG_OUT("[ONCLS] %s has a pUv %p or pCon %p null\n", _pBofUvCallback_X->Name_S.c_str(), _pBofUvCallback_X->pUvConnection_X,
		        _pBofUvCallback_X->pUvConnection_X ? _pBofUvCallback_X->pUvConnection_X->pIBofIoConnection : nullptr);
		// pIBofIoConnection = nullptr;
	}
	else
	{
	...
	DBG_OUT("[ONCLS] %s has a cb %p pUv %p -> pCon %p pIo %p/%p pConnect %p Remain %d \n", _pBofUvCallback_X->Name_S.c_str(), _pBofUvCallback_X, _pBofUvCallback_X->pUvConnection_X,
	        _pBofUvCallback_X->pUvConnection_X ? _pBofUvCallback_X->pUvConnection_X->pIBofIoConnection : nullptr,
	        _pBofUvCallback_X->pUvConnection_X ? _pBofUvCallback_X->pUvConnection_X->pUvIoChannel : nullptr, _pHandle,
...
//No		BOF_ASSERT(It != mUvDisconnectingConnectionCollection.end() );
		if (It != mUvDisconnectingConnectionCollection.end())
		{
			...
			DBG_OUT("[ONCLS] %s pUv %p pCon %p has been erased->%d remains\n", _pBofUvCallback_X->Name_S.c_str(), _pBofUvCallback_X->pUvConnection_X,
			        _pBofUvCallback_X->pUvConnection_X ? _pBofUvCallback_X->pUvConnection_X->pIBofIoConnection : nullptr, mUvDisconnectingConnectionCollection.size());
		}
	}
...
#undef X
	...
	mUvWalkResult_S += BOF_NAMESPACE::Bof_Sprintf("%p %s[%c%c%c] %-8s %p '%s'\n", _pBofUvCallback_X, uv_is_active(_pHandle) ? "ON :" : "OFF:", "R-"[!(_pHandle->flags & UV__HANDLE_REF)],
	                                              "A-"[!(_pHandle->flags & UV__HANDLE_ACTIVE)], "I-"[!(_pHandle->flags & UV__HANDLE_INTERNAL)], pType_c, (void *) _pHandle,
	                                              _pBofUvCallback_X->Name_S.c_str());
...
BOFERR BofIoConnectionManager::S_SetIoChannelOption(bool _IsUdp_B, uv_handle_t *_pUvHandle_X, const BOF_IO_CHANNEL_PARAM &_rBofIoChannelParam_X)
{
	...
	if (_pUvHandle_X)                                                                                              // && (_pUvTcp_X->type == UV_TCP))
	{
		if (_IsUdp_B)
		{
			...
			{
				// DBG_OUT("### Sock multicast receiver %s Loop %s ttl %d interface %s broad %s\n", _rBofIoChannelParam_X.IO_CHANNEL.UDP.MulticastReceiver_B ? "true" : "false", _rBofIoChannelParam_X.IO_CHANNEL.UDP.MulticastLoop_B ? "true" : "false", _rBofIoChannelParam_X.IO_CHANNEL.UDP.Ttl_U32, InterfaceIpAddressComponent_X.IpAddress_S.c_str(), _rBofIoChannelParam_X.IO_CHANNEL.UDP.Broadcast_B ? "true" : "false");
